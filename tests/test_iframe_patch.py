...
# ---------------------------------------------------------------------------
# Additional tests generated by CodeRabbit Inc. – Framework: pytest
# These tests extend coverage around success/failure paths and wrapper behavior.
# ---------------------------------------------------------------------------

def test_enable_iframe_support_missing_browser_use_logs_error_and_returns_none(caplog):
    # Ensure none of the fake 'browser_use' modules are present to force ImportError
    ...
    assert result is None
    assert any("Failed to enable iframe support" in rec.message for rec in caplog.records), \
        "Expected an ERROR log when 'browser_use' modules are missing."


def test_enable_iframe_support_passes_session_to_patch_function(monkeypatch):
    # Install fakes and capture the exact session object received by the patch function
    iframe_mod, _ = make_fake_browser_use_modules()
    received = {}
    ...
    iframe_mod.patch_browser_use_with_iframe_support = _capture_session

    with isolated_import("iframe_patch") as iframe_patch:
        session_obj = object()
        out = iframe_patch.enable_iframe_support(session_obj)

    assert out == "CAPTURED"
    assert received.get("session") is session_obj, "enable_iframe_support must pass the session through verbatim."


def test_auto_patch_browser_use_invokes_enable_for_each_instance(monkeypatch):
    # Arrange
    _, fake_session_mod = make_fake_browser_use_modules()
    with isolated_import("iframe_patch") as iframe_patch:
        calls = []
        ...
        # Act
        iframe_patch.auto_patch_browser_use()
        a = fake_session_mod.BrowserSession()
        b = fake_session_mod.BrowserSession()

        # Assert
        assert calls == [a, b], "enable_iframe_support should be called once per instance construction."


def test_auto_patch_browser_use_is_safe_to_call_multiple_times(monkeypatch):
    # Calling auto_patch twice should not raise and should still enable once per instantiation (at minimum).
    _, fake_session_mod = make_fake_browser_use_modules()
    with isolated_import("iframe_patch") as iframe_patch:
        calls = []
        ...
        # Act - invoke twice deliberately
        iframe_patch.auto_patch_browser_use()
        iframe_patch.auto_patch_browser_use()

        inst = fake_session_mod.BrowserSession()

        # Assert - at least one enable call should occur; double-wrapping would increase the count.
        assert len(calls) >= 1 and inst in calls


def test_auto_patch_browser_use_preserves_init_args_and_kwargs(monkeypatch):
    # Verify original __init__ side effects on args/kwargs are preserved by the wrapper.
    _, fake_session_mod = make_fake_browser_use_modules()
    with isolated_import("iframe_patch") as iframe_patch:
        monkeypatch.setattr(iframe_patch, "enable_iframe_support", lambda self: None, raising=True)

        iframe_patch.auto_patch_browser_use()

        inst = fake_session_mod.BrowserSession(10, 20, foo="bar", baz=5)

        assert getattr(inst, "args", None) == (10, 20)
        assert getattr(inst, "kwargs", None) == {"foo": "bar", "baz": 5}
